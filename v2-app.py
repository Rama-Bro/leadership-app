...
if st.session_state.page == "code":
    st.title("리더십 자가진단 시작")
    code = st.text_input("고유 코드를 입력하세요")
    if st.button("다음") and code:
...
# -------------------------------
    ...
    st.title("문항 응답")
    st.markdown("각 문항에 대해 가장 적절한 응답을 선택해주세요.")
    ...
    for i, q in enumerate(questions, 1):
        ...
        with col1:
            st.caption("동의하지 않는다")
        with col2:
            st.radio(f"{i}. {q}", [1, 2, 3, 4, 5], key=f"q{i}", horizontal=True, label_visibility="collapsed")
        with col3:
            st.caption("동의한다")
...
# -------------------------------
    ...
    for idx, area in enumerate(area_names):
        start = idx * 4
        scores = [st.session_state[f"q{i+1}"] for i in range(start, start + 4)]
